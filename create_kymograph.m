function [kymo] = create_kymograph(data, plot_radial, binsize, num_Naan_thr)
    arguments
        data;
        % Set plot_radial to 1 for radially expanding clusters, or to 0 for rectangular clusters
...
if plot_radial
    ...
    % If plot_radial is 0, make a distance map using the x-coordinate of each measurement
    [Y,X]=meshgrid(1:size(firstImageSlice,1),1:size(firstImageSlice,2));
    disFromCentroidYX=Y;
    kymoLength=size(firstImageSlice,2);%set the spatial extent of the kymograph
    kymo=nan(numberOfSlices,kymoLength); %predefine the kymograph with NaNs
    ...
    for t=1:1:numberOfSlices
        ...
        rCounter=1;
        %inner loop is over bins
        for y=binsize:binsize:kymoLength

            if y<=binsize %first bin is from the center to the binsize

                kymo(tcounter,1)=nanmean(thisArraySlice((disFromCentroidYX>=0)&(disFromCentroidYX<=binsize)));
            else %successive bins are between inner and outer sizes
                % Check for nan measurements 
                elementIsNan=isnan(thisArraySlice(:,y));
                numberOfElements=length(elementIsNan);
                numberOfNans=sum(elementIsNan);
                %numberOfNans returns the number of measurements
                %that are outside of the tissue at a given distance
                ...
                if numberOfNans/numberOfElements<=num_Naan_thr
                    ...
                    %points at that distance are outside the extent of
                    %the tissue
                    kymo(tcounter,rCounter)=nanmean(thisArraySlice(:,y));
                end
            end
            rCounter=rCounter+1;
        end
